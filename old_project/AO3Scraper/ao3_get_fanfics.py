######
#THIS IS WORKING WITH THE CSV NOW BUT LET ME CHECK
#
...
def write_fic_to_csv(fic_id, only_first_chap, lang, include_bookmarks, metadata_only, writer, errorwriter, headers):
    ...
    try:
        ...
        if response.status_code != 200:
            raise Exception(f"Bad HTTP status: {response.status_code}")

        soup = BeautifulSoup(response.text, 'html.parser')

        title = soup.find('h2', {'class': 'title'}).get_text(strip=True)
        authors = [a.get_text(strip=True) for a in soup.find_all('a', rel='author')]
        tags_divs = soup.find_all('dd')
        tags = [[], [], [], [], [], []]
        for i, tagset in enumerate(tags_divs[:6]):
            tags[i] = [a.get_text(strip=True) for a in tagset.find_all('a')]


        words = soup.find('dd', {'class': 'words'})
        words = words.get_text(strip=True) if words else ''
        kudos = soup.find('dd', {'class': 'kudos'})
        kudos = kudos.get_text(strip=True) if kudos else ''
        bookmarks = soup.find('dd', {'class': 'bookmarks'})
        bookmarks = bookmarks.get_text(strip=True) if bookmarks else ''
        comments = soup.find('dd', {'class': 'comments'})
        comments = comments.get_text(strip=True) if comments else ''
        hits = soup.find('dd', {'class': 'hits'})
        hits = hits.get_text(strip=True) if hits else ''
        published = soup.find('dd', {'class': 'published'})
        published = published.get_text(strip=True) if published else ''
        status = ''
        status_date = ''
        chapters = ''

        #had to modify this part for getting the text to get the whole story
        chapters_div = soup.find("div", {"id": "chapters"})
        if not chapters_div:
            raise Exception("Could not find chapters div")

        userstuff_blocks = chapters_div.find_all("div", {"class": "userstuff"})
        all_texts = []
        for block in userstuff_blocks:
            all_texts.append(block.get_text("\n", strip=True))

        raw_text = "\n\n=== CHAPTER BREAK ===\n\n".join(all_texts)
        chaptertext = raw_text  # keep UTF-8 chars

        #print(f"Extracted text length: {len(chaptertext)}")

        #making .txt just in case csv transfer doesn't work
        txt_path = f"debug_fic_{fic_id}.txt"
        with open(txt_path, "w", encoding="utf-8") as dbg:
            dbg.write(chaptertext)
        print(f"[INFO] Saved backup text: {txt_path}")

        #writing to fanfics.csv
        row = [
            fic_id,  # work_id
            title,  # title
            ", ".join(authors),  # author
            ...
            bookmarks,  # bookmarks
            hits,  # hits
            '', '',  # all_kudos, all_bookmarks (empty)
            chaptertext  # body
        ]

        row = [str(cell).encode('utf-8', errors='ignore').decode('utf-8') for cell in row]

        writer.writerow(row)
        print(f"Successfully wrote {fic_id} to CSV ({len(chaptertext)} chars).")

    except Exception as e:
        print(f"[ERROR] {fic_id}: {e}")
...
def main():
    ...
    with open(csv_out, 'a', newline="", encoding='utf-8-sig') as f_out:
        writer = csv.writer(f_out, quoting=csv.QUOTE_ALL)
        with open(os.path.join(os.path.dirname(csv_out), "errors_" + os.path.basename(csv_out)), 'a',
                  newline="", encoding='utf-8-sig') as e_out:
            errorwriter = csv.writer(e_out, quoting=csv.QUOTE_ALL)
            ...
            if is_csv:
                ...
                with open(csv_fname, 'r+', newline="") as f_in:
                    ...
                    if restart == '':
                        ...
                        for row in reader:
                            ...
                            if found_restart:
                                write_fic_to_csv(row[0], only_first_chap, lang, include_bookmarks, metadata_only,
                                                 writer, errorwriter, headers)
                                time.sleep(delay)
                            else:
                ...
                for fic_id in fic_ids:
                    write_fic_to_csv(fic_id, only_first_chap, lang, include_bookmarks, metadata_only, writer,
                                     errorwriter, headers)
                    time.sleep(delay)
